...
! and their load/destruction routines for the ODSSU algorithm
!
! PUBLIC DATA:
...
MODULE ODSSU_TauCoeff
  ...
  ! The shared data
  PUBLIC :: TC
  ! Structure defined in ODSSU_Define
  PUBLIC :: ODSSU_TauCoeff_type
  ...
  '$Id: ODSSU_TauCoeff.f90 22707 2012-11-21 21:09:10Z paul.vandelst@noaa.gov $'
  ...
  ! (AtmAbsorption) model
  ! --------------------------------------
  TYPE(ODSSU_TauCoeff_type), SAVE, ALLOCATABLE, TARGET :: TC(:)
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Destroy_TauCoeff( Process_ID,   &  ! Optional input
    ...
    DO n = 1, SIZE(TC)
      Destroy_Status = ODSSU_Destroy_TauCoeff( TC(n), &
                                               Message_Log=Message_Log )
